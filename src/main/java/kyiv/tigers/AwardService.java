 ...
 */
@Service
public class AwardService implements Importer{

    private static final Logger logger = Logger.getLogger(TenderService.class.getName());
    private static final String SELECT_DATA = stringFromFile("sql/publicbid/selectAwards.sql");
    private static final String COUNT_QUERY = stringFromFile("sql/publicbid/selectCountAwards.sql");
    private static final String INSERT_DATA = stringFromFile("sql/opentender/insertAwards.sql");
    ...
    public AwardService(@Qualifier("publicbidJdbcTemplate") NamedParameterJdbcTemplate publicbidJdbcTemplate,
    ...
    public boolean start(){
        ...
        try{
            ...
            while((values = selectValues(PageRequest.of(page, pageSize))).size() != 0){
                allUpdated += Arrays.stream(insertValues(values)).sum();
                page++;
                logger.info("Awards: " + allUpdated + "/" + count + "(" + (100 * allUpdated/count) + "%)");
            }
